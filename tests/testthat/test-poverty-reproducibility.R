...
test_that("poverty reproducibility preflight passes with complete variables", {
  ...
  expect_true(all(c("variable", "role", "present", "class", "missing_n", "non_missing_n", "issue") %in% names(preflight)))
...
test_that("reproducibility runner honors custom survey design variables", {
  data <- tibble::tibble(
